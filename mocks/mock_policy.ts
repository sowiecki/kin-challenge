...
export const genMockFile = (length = NUMBER_OF_POLICY_NUMBERS) => new Array(length).fill(null).map((_) => transformNumberToOCR(genMockPolicyNumber()).concat(EMPTY_LINE).join('\n')).join('\n');
